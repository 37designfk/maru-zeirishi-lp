...
- **短縮URLの注意**: `shortenFormUrl` は呼ぶたびに新しい `forms.gle/...` エイリアスを発行する（`buildForm` 再実行のたびに別の短縮URLが実行ログに出る）。ただし**回答URL本体（`/forms/d/e/.../viewform`）は不変**で、過去に発行した短縮URLも同じviewformに解決され有効なまま。LP掲載中の短縮URL（`forms.gle/HNLzNHERpsepxAMX8`）はそのまま使い続けてよく、再実行のたびにLPを差し替える必要はない。
