...
5. 回答URL（短縮）が変わった場合は `index.html` のボタン `href`（`.cta-btn`）を差し替える。

## LP側

`index.html` のCTA（`#contact`）に Googleフォームへのボタンリンクを設置済み。

```html
<a class="cta-btn" href="https://forms.gle/HNLzNHERpsepxAMX8" target="_blank" rel="noopener">無料で相談・お見積りを依頼する</a>
```
